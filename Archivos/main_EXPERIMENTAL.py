...
if seccion == 'Datos':
    ...
    # Toggle para cargar tabla personalizada
    ...
    if datos_especificos:
        if 'Datos_usuario' not in st.session_state:
            ...
            st.session_state['Archivo_cargado']=Datos_usuario
        else:
            if 'Archivo_cargado' not in st.session_state:
                _=None
            else:
                Datos_usuario = st.session_state['Archivo_cargado']

        # Validar si el usuario cargó el archivo
        if Datos_usuario is not None:
            try:
                # Leer el archivo cargado
                Datos = pd.read_excel(Datos_usuario, index_col=0)
            ...
            # Advertencia si no se carga ningún archivo
            st.warning("Por favor, carga un archivo para continuar.")
            st.stop()
    else:
        # Usar el archivo predefinido
        Datos = pd.read_excel(
            'Archivos/Datos_climatologicos_Santa_Fe_2019.xlsx', index_col=0
        )
        st.success("Se han cargado los datos predeterminados de Santa Fe 2019.")
